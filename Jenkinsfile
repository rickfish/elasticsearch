...
def TEMPLATES = ['','es_stewardship_prod']
def NOTIFICATION = "Elasticsearch@bcbsfl.com"
...
pipeline {
    ...
    stages {
        stage('Validate Prod - Ticket') {
            steps {
                script {
                    if(params.DEPLOYMENT_TEMPLATE == "es_stewardship_prod" && env.BRANCH_NAME == MASTER_BRANCH  ){
